...
const Salaries = () => {
  ...
  const calculateSalary = () => {
    ...
    const etfRate = parseFloat(formData.etfRate) || 12;
  ...
  const handleEdit = async (salary) => {
    if (salary.status === "Paid") {
      toast.error("Cannot edit a salary that has already been paid");
      return;
    }
    ...
    setFormData({
      ...
      otHours: salary.otHours?.toString() || "0",
      otRate: salary.otRate?.toString() || "1.5",
      epfRate: salary.epfRate?.toString() || "8",
      etfRate: salary.etfRate?.toString() || "12",
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response;
      if (editingSalary) {
        response = await axios.put(`/api/salaries/${editingSalary._id}`, payload);
      } else {
        response = await axios.post("/api/salaries", payload);
      }

      if (response.data.success) {
        toast.success(`Salary ${editingSalary ? 'updated' : 'added'} successfully`);
        setShowForm(false);
        setEditingSalary(null);
        setFormData({
          employeeId: "",
        ...
        toast.error(response.data.message || `Failed to ${editingSalary ? 'update' : 'add'} salary`);
      }
    } catch (error) {
      ...
      if (error.response?.data?.message) {
        ...
        toast.error(`Failed to ${editingSalary ? 'update' : 'add'} salary. Please check your connection and try again.`);
      }
    } finally {
      setSubmitting(false);
    }
              ...
              <form onSubmit={handleSubmit}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <select
                      ...
                      disabled={submitting || editingSalary?.status === "Paid"}
                    >
                ...
                <div className="flex justify-end space-x-3 pt-4">
                  ...
                  <button 
                    ...
                    disabled={submitting}
                  >
                    {submitting ? (editingSalary ? "Updating..." : "Adding...") : (editingSalary ? "Update Salary" : "Add Salary")}
                  </button>
                </div>
              </form>
            </div>
